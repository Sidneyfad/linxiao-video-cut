  ...
  send(text) {
    ...
    this._watchdog = setInterval(() => {
      ...
      if (idleMs > 60_000) {
        ...
        this.busy = false;
        const stderrTail = this.stderrBuffer.slice(-15);
        const stderrText = stderrTail.length
          ? "子进程最近 stderr：\n  " + stderrTail.join("\n  ")
          : "（子进程未输出任何 stderr — 多半是 SDK 还没建立 API 连接就卡住了）";
        try {
          this.onEvent({
            type: "result",
            subtype: "stuck",
            ...
            duration_api_ms: 0,
            num_turns: 0,
            result: `Agent 60s 内零进展。\n\n${stderrText}\n\n常见原因：\n- 该模型不支持 Claude 协议的 tool_use / streaming（OpenRouter 上仅 anthropic/* 系列可靠；MiniMax / Kimi / 通义等的 /anthropic 兼容层往往只支持简单消息）\n- 系统提示约 22KB，部分模型上下文窗口不够\n- 网络层 TLS / DNS 阻塞\n\n建议：先去 ⚙ 设置换成 deepseek-v4-flash 或官方 Claude 验证，再排查目标模型`,
