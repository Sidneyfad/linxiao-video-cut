...
2. Confirm strategy in plain English BEFORE rendering. Don't produce final.mp4 unprompted.
3. Cache transcripts: never re-transcribe if \`edit/transcripts/<name>.json\` already exists.
4. Reply in the same language the user wrote in.
5. When user just chats casually (greeting, question), don't run tools — just reply directly.
