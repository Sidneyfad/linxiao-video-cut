  ...
  start() {
    ...
    this.q = query({
      ...
      options: {
        ...
        // Force the SDK's bundled CLI into verbose logging mode + write to
        // a known file so we can inspect via /api/debug-log even when the
        // stderr callback doesn't fire (turns out the bundled CLI is mostly
        // silent on stderr in normal operation; debug logs go to file).
        debug: true,
