...
export function buildAgentEnvAndOptions() {
  ...
  // Enable Anthropic SDK's internal debug logging — this is a separate
  // mechanism from the bundled CLI's `debug: true` option and does fire on
  // every HTTP request/retry/error. Without it, even a totally-stuck SDK
  // emits zero stderr. With it, you get HTTP timing, retry attempts, and
  // error details that pinpoint where exactly things wedge.
