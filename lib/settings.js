...
// True if the value looks like a placeholder rather than a real API key.
// Used to refuse work upfront instead of letting the agent burn turns and
// then "rationalize" a fallback (e.g. local Whisper, which the skill forbids).
export function looksLikePlaceholder(v) {
  const s = String(v || "").trim();
  if (!s) return true;
  if (s.length < 20) return true;
  const lower = s.toLowerCase();
  return lower.startsWith("sk_test_")
...
export function buildAgentEnvAndOptions() {
  ...
  // CLAUDE_CODE_ENTRYPOINT="local-agent" is the value the bundled CLI uses
  // for headless agent embedding (vs "claude-desktop" for the GUI). Setting
  // anything else triggers two mystery functions Kvq() + xVq() during
  // setup() — those are the death-after-getSkills we kept seeing on Render.
