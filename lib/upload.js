...
// Cancel an in-progress upload. Removes the entire .upload-<uploadId>/ dir
// so partial chunks don't squat on disk. Idempotent — safe to call after
// the upload already finished (returns 404 cleanly).
export function cancelUpload(req, res, session) {
  const filename = decodeURIComponent(req.params.filename);
  const uploadId = req.params.uploadId;
  if (!filename || !uploadId || !/^[a-fA-F0-9-]{8,64}$/.test(uploadId)) {
    res.status(400).json({ error: "invalid params" });
    return;
  }
  const dir = path.join(session.dir, `.upload-${uploadId}`);
  let removed = false;
  if (fs.existsSync(dir)) {
    try {
      fs.rmSync(dir, { recursive: true, force: true });
      ...
      console.warn("[upload] failed to remove cancel dir:", e.message);
    }
  }
  // Drop any registry entry too. We have to walk because uploads.delete needs
  // the exact key but it's keyed by filename + uploadId.
  for (const key of [...uploads.keys()]) {
    if (key.endsWith(`|${filename}|${uploadId}`)) uploads.delete(key);
  }
